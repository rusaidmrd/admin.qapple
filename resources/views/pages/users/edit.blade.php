        ...
        <div class="bg-white shadow rounded-md sm:rounded-lg p-8">
            <form action="{{ route('users.update',$user->id) }}" method="POST">
                ...
                <x-input.group label="Name" for="name" :error="$errors->first('name')">
                    <input name="name" id="name" class="form-input" type="text" placeholder="Eg : Jhon Steve" value="{{ old('email', $user->name) }}">
               </x-input.group>

               <x-input.group label="Email address" for="email" :error="$errors->first('email')">
                    <input name="email" id="email" class="form-input" type="email" placeholder="Eg : jhonsteve@gmail.com" value="{{ old('email', $user->email) }}">
               </x-input.group>
